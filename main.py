...
def run_sim(active):
    ...
    for round_num in range(rounds_per_iteration):
        ...
        if active:
            ...
            lit_regions = (region_data[1] > light_threshold * cells_per_region)[
                ..., None
            ]
...
def run_main():
    ...
    for iteration_num in tqdm(range(iterations)):
        ...
        light_log[iteration_num] = np.sum(
            region_data[1] > light_threshold * cells_per_region * light_decay
        )
...
def display_data_local():
    ...
    for i in range(3):
        for j in range(3):
            ...
            ax.axvline(
                reshaped[:, i, j].mean(), color="k", linestyle="dashed", linewidth=1
            )
...
def display_data_global():
    ...
    for i in range(detail):
        for j in range(detail):
            ...
            for k in range(3):
                test_data_arr[1, i, j, k] = (
                    100 * np.sum(test_weights[k]) / reshaped.shape[0]
                )
    ...
    for typ in range(3):
        ...
        ax.plot_surface(
            x, y, test_data_arr[0, ..., typ], rstride=1, cstride=1, cmap=cm.coolwarm
        )
        ax.set_xlabel("sig")
        ax.set_ylabel("eng")
        ...
        ax.plot_surface(
            x, y, test_data_arr[1, ..., typ], rstride=1, cstride=1, cmap=cm.coolwarm
        )
        ax.set_xlabel("sig")
        ax.set_ylabel("eng")
        ax.set_title(["eng", "sig", "lgt"][typ])
